class Timer {
  ...
  timer(end_event_name = "end") {
    const [prev_minutes, prev_seconds] = this.display.innerHTML
      .split(":")
    ...
    const [current_minutes, current_seconds] = [
      Math.floor(total_seconds / 60),
      total_seconds % 60,
    ];
    this.display.innerHTML = `${current_minutes}:${
      current_seconds > 9 ? current_seconds : "0" + current_seconds
    }`;
    return;
  }
